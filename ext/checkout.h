...
/* variable defs */
#ifndef checkout_scan_c
extern VALUE super_ParseError, mcheckout, super_checkout;
...
VALUE checkout_inline(VALUE, char *, char *, VALUE);
VALUE checkout_inline2(VALUE, VALUE, VALUE);
VALUE checkout_attribute_parser(int, VALUE, char *, char *);
VALUE checkout_attributes(VALUE, VALUE);
VALUE checkout_link_attributes(VALUE, VALUE);
VALUE red_parse_title(VALUE, VALUE, VALUE);
VALUE checkout_transform(VALUE, char *, char *, VALUE);
VALUE checkout_transform2(VALUE, VALUE);
...
#define TRANSFORM(T) \
  if (p > reg && reg >= ts) { \
    VALUE str = checkout_transform(self, reg, p, refs); \
